...
const ClientHistory = ({ dict }: { dict: any }) => {
    ...
    const tradeHeaders = [
        "symbol", "ticket", "type", "entry", "exit", "profit", "unit", 'leverage', "tp", "sl", "state", "time"
    ]
            ...
            <Card className='w-full h-full'>
                <CardHeader className='flex items-center justify-center py-2'>
                    <div className='w-full flex items-center justify-between mx-3 mt-3'>
                        <Drawer>
                            <DrawerTrigger className='flex items-center w-6'><LuCalendar className='text-xl' /></DrawerTrigger>
                ...
                <CardContent className='w-full px-2'>
                    <Tabs defaultValue="trade" className="w-full">
                        <TabsList className='w-full flex items-center justify-center'>
                            <TabsTrigger className='w-32' onClick={() => tabChanger('trade')} value="trade">{dict.history.trade}</TabsTrigger>
                            <TabsTrigger className='w-32' onClick={() => tabChanger('deal')} value="deal">{dict.history.deal}</TabsTrigger>
                            <TabsTrigger className='w-32' onClick={() => tabChanger('order')} value="order">{dict.history.order}</TabsTrigger>
                            <TabsTrigger className='w-32' onClick={() => tabChanger('transaction')}value="transaction">{dict.history.transaction}</TabsTrigger>
                        </TabsList>
                        <TabsContent value="trade">
                            <div className='h-[62vh] sm:h-[270px] overflow-scroll'>
                                {trades.length ? (
                                    <HistoryTable items={trades} headers={tradeHeaders} dict={dict} isOrder={false} />
                                ) :
                                    (<Empty dict={dict} />)}
                            </div>
                        </TabsContent>
                        <TabsContent value="deal">
                            <div className='h-[62vh] sm:h-[270px] overflow-scroll'>
                                {deals.length ? (
                                    <HistoryTable items={deals} headers={dealHeaders} dict={dict} isOrder={false} />
                                ) :
                                    (<Empty dict={dict} />)}
                            </div>
                        </TabsContent>
                        <TabsContent value="order">
                            <div className='h-[62vh] sm:h-[270px] overflow-scroll'>
                                {orders.length ? (
                                    <HistoryTable items={orders} headers={orderHeaders} dict={dict} isOrder={true} />
                                ) :
                                    (<Empty dict={dict} />)}
                            </div>
                        </TabsContent>
                        <TabsContent value="transaction">
                            <div className='h-[62vh] sm:h-[270px] overflow-scroll'>
                                {transactions.length ? (
                                    <HistoryTable items={transactions} headers={transactionsHeaders} dict={dict} isOrder={false} />
                                ) :
                                    (<Empty dict={dict} />)}
                            </div>
                        </TabsContent>
